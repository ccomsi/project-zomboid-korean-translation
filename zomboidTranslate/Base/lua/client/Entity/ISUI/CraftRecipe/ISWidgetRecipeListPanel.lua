...
function ISWidgetRecipeListPanel:createChildren()
    ...
    self.recipeListPanel.doDrawItem = function(_self, _y, _item, _alt)
        ...
        if craftRecipe then
            ...
            local starIconY = _y+detailsY + (FONT_HGT_HEADING/2) - (LIST_SUBICON_SIZE/2);
            if isFavourite then
                _self:drawTextureScaledAspect(_self.starSetTexture, detailsLeft + recipeNameWidth + UI_BORDER_SPACING, starIconY, LIST_SUBICON_SIZE, LIST_SUBICON_SIZE, color.a, color.r, color.g, color.b);
                --else
                --    _self:drawTextureScaledAspect(_self.starUnsetTexture, detailsLeft + recipeNameWidth + UI_BORDER_SPACING, starIconY, LIST_SUBICON_SIZE, LIST_SUBICON_SIZE, color.a, color.r, color.g, color.b);
            end
