...
function ISUsersList:initialise()
    ...
    self.bannedIPs = ISButton:new((UI_BORDER_SPACING+1)*2+ btnWid, self:getHeight() - UI_BORDER_SPACING - BUTTON_HGT - 1, btnWid, BUTTON_HGT, getText("IGUI_UserList_BannedIPs"), self, ISUsersList.onClick);
    self.bannedIPs.internal = "BANNEDIP";
    ...
    local titleFilter = ISLabel:new((UI_BORDER_SPACING+1)*3 + btnWid*2, self:getHeight() - UI_BORDER_SPACING - BUTTON_HGT - 1, searchHeight, getText("IGUI_UsersList_Filter"), 1, 1, 1, 1, UIFont.Small, true)
    titleFilter:initialise()
    titleFilter:instantiate()
    self:addChild(titleFilter)

    self.searchEntry = ISTextEntryBox:new("", (UI_BORDER_SPACING+1)*3 + btnWid*2 + titleFilterWid, self:getHeight() - UI_BORDER_SPACING - BUTTON_HGT - 1, 150 , searchHeight)
    ...
    for i=0,users:size()-1 do
        ...
        if searchWord == ''
            or (not (searchWord == '') and (
                string.find(user:getUsername(), searchWord) ~= nil or
                (not getSteamModeActive() and string.find(user:getIpBanned(), searchWord) ~= nil) or
                (getSteamModeActive() and string.find(user:getSteamIdBanned(), searchWord) ~= nil) )) then
...
function ISUsersList:drawDatas(y, item, alt)
    ...
    if item.item:isInWhitelist() then
        local text = getText("IGUI_UsersList_DetailsRole", item.item:getRole():getName());
        if not getSteamModeActive() and item.item:getIpBanned() ~= nil and item.item:getIpBanned() ~= '' then
            text = text..', IP banned';
        end
        if getSteamModeActive() and item.item:getSteamIdBanned() ~= nil and item.item:getSteamIdBanned() ~= '' then
            text = text..', SteamID banned';
        end
        self:drawText(text, self:getWidth() - 330, y + 4, 1, 1, 1, a, self.font);
...
function ISUsersList:onClick(button)
    ...
    if button.internal == "BANNEDIP" then
...
function ISUsersList:onClickOption(item, action)
    if action == "Teleport" or action == "TeleportToHim" or action == "Kick" or action == "Ban" or action == "UnBan" or action == "BanIP" or action == "UnBanIP" or action == "BanSteamID" or action == "UnBanSteamID" or
            action == "ResetTOTPSecret" or action == "ResetPassword" then
        if (action == "BanSteamID" or action == "UnBanSteamID") then
            networkUserAction(action, item:getSteamid(), "");
        else
...
function ISUsersList:doContextMenu(item, x, y)
    ...
    if item:isOnline() then
        ...
        if self.player:getRole():haveCapability(Capability.KickUser) then
            ...
            if item:getRole():haveCapability(Capability.CantBeKicked) then
                kickButton.notAvailable = true;
                local tooltip = ISWorldObjectContextMenu.addToolTip();
    ...
    if self.player:getRole():haveCapability(Capability.BanUnbanUser) then
        ...
        if item:getRole():getName() == 'banned' then
            ...
            if item:getRole():haveCapability(Capability.CantBeBannedByUser) then
                banButton.notAvailable = true;
                local tooltip = ISWorldObjectContextMenu.addToolTip();
